...
dat<-read_csv("data/CDC_CSSE_WeeklyDeaths_incl_assigned.csv")
...
graphs<-FALSE #IF you want to display all graphs
gold_test<-FALSE #To check outlier detection against gold standard data. Make sure gold standard data is up to date
...
spline_function<- function(date,incidD,weights=rep(1,length(incidD)),knots=50){
  mod.ss <- ss(date,incidD, w= weights, nknots = knots)
